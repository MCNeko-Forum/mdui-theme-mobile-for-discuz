...
## `mdui_ladyload`：图片懒加载开关

启用后，帖子内容图片、版块帖子封面、导读与圈子列表缩略图、图片排行榜等滚动密集区域的图片会带上原生 `loading="lazy"` 属性，由浏览器在图片即将进入视口时才加载，减少首屏流量与请求数。
...
- 留空即关闭（默认），关闭时页面不输出任何懒加载属性。
- 使用的是浏览器原生懒加载（`loading="lazy"`），无额外 JavaScript；低端内核不支持该属性时会自动退化为立即加载，不影响显示。
- 头像与小图标（表情、图章等）不参与懒加载。
- 修改后如未生效，请清理模板编译缓存。

***

## `mdui_block_cache`：门户首页 DIY 模块缓存时间

控制门户首页（portal.php）手机版中，幻灯片、头条、热帖、最新回复、精华帖子、最新帖子这六个「论坛帖子」类 DIY 模块的数据缓存时长（秒）。
...
| ------ | ---------------------------------------- |
| 留空     | 回退为默认 `30` 秒                            |
| `0`    | 不缓存：每次访问都即时重建全部帖子模块（数据最实时，但每次请求多出约 60 条 SQL 写查询，高并发下会显著拖慢首页，仅调试时使用） |
| 正整数    | 模块数据超过该秒数后，由下一个请求触发重建（受 Discuz 进程锁节流，同一时刻只有一个请求在重建，其余请求继续使用旧数据，不会阻塞） |

### 注意

- 该变量只影响门户首页的帖子类 DIY 模块；后台 DIY 保存时所有模块仍会立即刷新，不受此值影响。
- 官方推荐的默认值 `30` 在保证首页约 0.03 秒响应的同时，数据最多延迟 30 秒。
- 修改后需要更新缓存（后台 → 工具 → 更新缓存）才会生效，因为主题变量会缓存进 `style_default` 系统缓存。
- 高并发站点请勿长期使用 `0`。
